...
def _fr_radar_shapes_features(rgba) -> list:
    ...
    Chaikin ×2 (≈ B-spline quadratique, beau lissé). Brièvement passé à ×1 (v1.3.103) pour la
    perf, puis restauré à ×2 (v1.3.103) une fois le rendu radar en couches-frames (le nombre de
    ...
    for k in range(1, nb + 1):
        ...
        for ring in _fr_marching_rings(mask):
            ...
            if len(dp) < 3:
                continue
            sm = _fr_ring_chaikin(dp, 2)   # v1.3.103 : ×2 restauré (beau lissé) — la perf ne dépend plus des sommets
